...
def high_level_func(stub, post_id):
    #retrieve post
    retrieve_post_content(stub, post_id)

    #retrieve comment, sorting by score and limiting to 1 to get the top comment
    request = reddit_pb2.CommentsRequest(post_id=post_id, limit=1, sort_by=reddit_pb2.CommentsRequest.SCORE)
    comments_response = stub.RetrieveComments(request)

    if not comments_response.comments:
        print(f"No comments found for post {post_id}.")
        return None

    #retrieve the most upvoted comment
    top_comment = comments_response.comments[0]

    #expand the top comment to get its replies, limiting to 1 to get the top reply
    expand_request = reddit_pb2.ExpandRequest(comment_id=top_comment.id, comment_num=1)
    expanded_response = stub.ExpandCommentBranch(expand_request)

    if not expanded_response.comments:
        print(f"No replies found for the top comment {top_comment.id}.")
        return None

    #return the top reply
    return expanded_response.comments[0]
...
class TestRetrieveTopReplyOfTopComment(unittest.TestCase):
    def setUp(self):
        #create a mock for the gRPC stub
        self.mock_stub = MagicMock(spec=reddit_pb2_grpc.RedditServiceStub)
    ...
    #positive test for expand most upvoted comment
    ...
    #negative test for expand most upvoted comment
    #most upvoted so limit == 1
